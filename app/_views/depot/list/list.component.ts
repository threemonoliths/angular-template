...
export class DepotListComponent implements OnInit {
    ...
    public depotList:Array<Depot>;
    private selectedDepot = new Depot();
    ...
    loadPage(model, sort, page, per_page): any{
        return this.depotService.listOnePage(model,sort,page,per_page).then(result => { this.depotList = result['items'];
    ...
    //新增按钮事件
    newDepot(): void {
        this.depotService.isUpdate=false;
        this.router.navigateByUrl('/home/depot/form');
    }
    
    //更新按钮事件
    updateDepot(): void {
        if (this.selectedDepot.id!=null){
            this.depotService.isUpdate=true;
            this.depotService.initUpdate(this.selectedDepot)
                .then(result => this.depotService.updateDepot = result)
                .then(() => this.router.navigateByUrl('/home/depot/updateform')).catch((error)=>
                this.eventbusService.addMsgs({severity:'error', summary:'程序运行异常', detail:error})); 
        } else {
            this.confirmationService.confirm({
                message: '请选择一个仓库信息进行修改！',
                accept: () => {
                }
            });
        }
    ...
    delete(): void {
        if (this.selectedDepot.id!=null){
            this.confirmationService.confirm({
                message: '确定要删除仓库信息：'+this.selectedDepot.DepotName+' 吗？',
                accept: () => {
                    this.depotService.delete(this.selectedDepot.id)
                        .then(()=>  this.loadPage(this.model,this.sort,1,this.recordsPerPage).then(() =>
                        this.eventbusService.addMsgs({severity:'success', summary:'删除仓库信息成功', detail:'仓库名称: '+this.selectedDepot.DepotName}))).then(()=>this.dt.reset())
                }
            });   
        } else {
            this.confirmationService.confirm({
                message: '请选择一个仓库信息进行删除！',
                accept: () => {
                }
            });
        }
