...
import { Godownentry } from '../../../_domains/godownentry';
...
export class GodownentryListComponent implements OnInit {

    public GodownentryList:Array<Godownentry>;
    private selectedGodownentry = new Godownentry();
    ...
    //面包屑组件导航相关数据
    labelstr = '合同管理';
    urlstr = frontendUrl + '/#/home/godownentry';
    
    ngOnInit() {
        this.eventbusService.addCrumb(this.labelstr,this.urlstr);
        //初始化默认分页对象
        this.sort.field='BillNo';
    ...
    loadPage(model, sort, page, per_page): any{
        return this.godownentryService.listOnePage(model,sort,page,per_page).then(result => { this.GodownentryList = result['items'];
  ...
  newa(){
        this.godownentryService.isUpdate=false;
        this.router.navigateByUrl('/home/godownentry/form');
    }
    
    //更新按钮事件
    update(): void {
      
        if (this.selectedGodownentry.id!=null){
            this.godownentryService.isUpdate=true;
            this.godownentryService.initUpdate(this.selectedGodownentry)
                .then(result => this.godownentryService.updateGodownentry = result)
                .then(() => this.router.navigateByUrl('/home/godownentry/updateform')).catch((error)=>
                this.eventbusService.addMsgs({severity:'error', summary:'程序运行异常', detail:error})); 
        } else {
    ...
    delete(): void {
        if (this.selectedGodownentry.id!=null){
            this.confirmationService.confirm({
                message: '确定要删除合同：'+this.selectedGodownentry.BillNo+' 吗？',
                accept: () => {
                    this.godownentryService.delete(this.selectedGodownentry.id)
                        .then(()=>  this.loadPage(this.model,this.sort,1,this.recordsPerPage).then(() =>
                        this.eventbusService.addMsgs({severity:'success', summary:'删除合同成功', detail:'合同号: '+this.selectedGodownentry.BillNo}))).then(()=>this.dt.reset())
                }
            });   
        } else {
            this.confirmationService.confirm({
