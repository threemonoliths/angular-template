...
export class UserListComponent implements OnInit {
    ...
    loadPage(model, sort, page, per_page): any{
        return this.userService.listOnePage(model,sort,page,per_page).then(result => { this.userList = result['items'];
    ...
    newUser(): void {
        ...
        this.router.navigateByUrl('/home/user/form');
    ...
    updateUser(): void {
        if (this.selectedUser.id!=null){
            ...
            this.userService.initUpdate(this.selectedUser)
                .then(result => this.userService.updateUser = result)
                .then(() => this.router.navigateByUrl('/home/user/updateform')).catch((error)=>
                this.eventbusService.addMsgs({severity:'error', summary:'程序运行异常', detail:error})); 
        } else {
    ...
    delete(): void {
        if (this.selectedUser.id!=null){
            this.confirmationService.confirm({
                message: '确定要删除用户：'+this.selectedUser.username+' 吗？',
                accept: () => {
                    this.userService.delete(this.selectedUser.id)
                        .then(()=>  this.loadPage(this.model,this.sort,1,this.recordsPerPage).then(() =>
                        this.eventbusService.addMsgs({severity:'success', summary:'删除用户成功', detail:'用户名称: '+this.selectedUser.username}))).then(()=>this.dt.reset())
                }
            });   
        } else {
            this.confirmationService.confirm({
