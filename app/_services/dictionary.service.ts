import { Injectable } from '@angular/core';
import { Http, URLSearchParams, RequestOptions, Headers } from '@angular/http';
import { Subject } from 'rxjs/Subject';
import 'rxjs/add/operator/map';
import 'rxjs/add/operator/toPromise';

import { Dictionary} from '../_domains/dictionary.domain';
import { baseUrl } from '../_services/eventbus.service';
//import { setTokenOptions } from '../_services/authentication.service';



@Injectable()
export class  DictionaryService {

  constructor(private http: Http) {}
   url = baseUrl+"/api/dict"

  
  listOnePage(model, sort, page, per_page) {
    let params = new URLSearchParams();
    params.set('Code', model.Code);
    params.set('Name', model.Name);
    params.set('Seq', model.Seq);
    return this.http.get(this.url+ `?page=${page}&per_page=${per_page}&sortfield=${sort.field}&sort=${sort.order}`, {search: params} )
               .toPromise().catch(this.handleError)
               .then(res => {return res.json()})           
  }

  

  add(c: Dictionary): Promise<any>{ 
    let params = new URLSearchParams();
    params.set('Code', c.Code);
    params.set('Name', c.Name);
    params.set('Key', c.Key);
    params.set('Parm', c.Parm);
    params.set('Value', c.Value ? String(c.Value) : '0');
    params.set('Seq', c.Seq ? String(c.Seq) : '0');
    return this.http.post(this.url,params)
               .map(response => response.json()).toPromise();
  }

  delete(id: any) {
    return this.http.delete(this.url+ `/${id}`)
               .toPromise();
  }

  isUpdate = false;
  updateDictionary :Dictionary = null;
  
  update(c: Dictionary): Promise<any>{
    let params = new URLSearchParams();
    params.set('Code', c.Code);
    params.set('Name', c.Name);
    params.set('Key', c.Key);
    params.set('Parm', c.Parm);
    params.set('Value', c.Value ? String(c.Value) : '0');
    params.set('Seq', c.Seq ? String(c.Seq) : '0');
    let id = c.id;
    return this.http.post(this.url + `/${id}`,params)
        .map(response => response.json()).toPromise();
  }

  initUpdate(c:Dictionary){
    console.log(c);
    let id = c.id;
    return this.http.get(this.url+ `/${id}`)
               .map(response => response.json()).toPromise();
  }
  
  //http异常处理函数
  private handleError(error: any): Promise<any> {
    console.log(error.status);
    return Promise.reject(error.message || error);
  }

  
  //验证编码重复
  public getByCode(un: string, id: any): Promise<any>{
    return this.http.get(this.url + `/${un}` + `?id=${id}` + '&isCheck=1')
               .toPromise().catch(this.handleError)
               .then(res => {return res.json()})          
  }
}


